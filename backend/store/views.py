...
def create_order(request):
    ...
    if method == 'bkash':
        try:
            ...
            if not id_token:
                return Response({'error': 'bKash token ?????? ??????'}, status=400)
            ...
            if not url:
                return Response({'error': 'bKash URL ?????? ??????', 'detail': pay_res}, status=400)
    ...
    if method == 'ssl':
        try:
            ...
            if res.get('status') != 'SUCCESS':
                return Response({'error': 'SSL Commerz session ???? ?????'}, status=400)
    ...
    if method == 'shurjopay':
        try:
            ...
            if not token:
                return Response({'error': 'Shurjopay token ?????? ??????'}, status=400)
            pay = requests.post(
            ...
            if not url:
                return Response({'error': 'Shurjopay URL ?????? ??????', 'detail': pay}, status=400)
            return Response({'id': order.id, 'payment_url': url})
        except requests.RequestException as e:
